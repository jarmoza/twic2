...
class TWiC_Poem:
    ...
    def ConvertToPlainText(self, output_directory, output_filename):
        ...
        with open(output_directory + output_filename, 'w') as plaintext_output_file:
    ...
    # Static TWiC_Poem threshholds for similarity testing
    ...
    def IsPoemSimilar(original_poem, compared_poem):
        ...
        for index in range(my_lines_count):
            if index < compared_lines_count and TWiC_Poem.PercentageLineMatch(my_lines[index], compared_lines[index], True) > TWiC_Poem.line_match_threshhold:
                match_index = index
                break
        ...
        return float(matches) / float(my_lines_count) > TWiC_Poem.poem_match_threshhold
    ...
    def GatherPoems(tei_source_dir, corpus_source_dir, remove_old_plaintext = False):
        ...
        for index in range(first_poem_number, last_poem_number + 1):
            all_poem_collection[index] = TWiC_Poem(input_directory + input_filename.format(index))
        ...
        for index in range(first_poem_number, last_poem_number + 1):
            ...
            for index2 in range(first_poem_number, last_poem_number + 1):
                ...
                if TWiC_Poem.IsPoemSimilar(all_poem_collection[index], all_poem_collection[index2]):
...
def main():
    ...
    for index in range(first_poem_number, last_poem_number + 1):
        ...
        poem = TWiC_Poem(input_directory + input_filename.format(index))
        title = poem.GetTitle()
